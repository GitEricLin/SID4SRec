...
from models.sid4srec import SID4SRec, CategoryContrastiveLearning
...
class Trainer:
    ...
    def _create_model(self):
        ...
        self.item_contrastive = CategoryContrastiveLearning(self.device, self.args)
        self.item_contrastive.to(self.device)
    ...
    def _train_one_epoch(self, epoch, only_bert_train=False, is_diffusion_train=True):
        ...
        for batch in prog_iter:
            ...
            item_cl_loss = self.item_contrastive(item_embeddings, torch.arange(self.args.item_size).to(self.device))
            
            # if epoch == 100 or epoch == 300 or epoch == 500:
            #     _ = self.item_contrastive.analyze_phi_effect(item_embeddings, item_id=5)
    ...
    def train(self):
        ...
        for epoch in trange(self.start_epoch, self.start_epoch + int(self.args.epochs), desc="Epoch"):
            ...
            early_stopping(np.array(scores[-3:]), self.model)
